	...
	if (playdev->sample_bytes == 2) {
		while (index < nSamples) {
			for (i = 0, n = index; n < nSamples; i += playdev->num_channels, n++) {
				ii = (int)(creal(cSamples[n]) / 65536);
				qq = (int)(cimag(cSamples[n]) / 65536);
		...
		while (index < nSamples) {
			for (i = 0, n = index; n < nSamples; i += playdev->num_channels, n++) {
				ii = (int)(creal(cSamples[n]) / 256);
				qq = (int)(cimag(cSamples[n]) / 256);
				if (!is_little_endian) {	// convert to big-endian
		...
		while (index < nSamples) {
			for (i = 0, n = index; n < nSamples; i += playdev->num_channels, n++) {
				ii = (int)creal(cSamples[n]);
				qq = (int)cimag(cSamples[n]);
